...
I then assessed the datatype of each column within the `recipes_and_ratings` dataset, ensuring that datatypes were consistent with the ones I needed.
...
Another feature that was interesting to graph was the `'n_ingredients'` column. Recipes with less ingredients may be considered healthier, as recipes with more simplicity might be less for enjoyment and more for dietary reasons. The graph of the distribution of recipes (count) given the number of ingredients needed for each recipe is shown below. We can see that the distribution is slightly skewed to the right, but a general bell-curve trend is followed. The mean number of ingredients is around 9.
...
I also wanted to see if recipe complexity (based on the number of steps or the number of ingredients) affected nutritional content. If there are a higher number of ingredients or steps, there may be a chance for higher PDV of carbohydrates, sugar, or fat. Below, I plotted the average nutritional content of recipes depending on the number of ingredients. In this plot, I excluded extreme outliers that were present within the `'pdv_carb'`, `'pdv_sugar'`, and `'pdv_fat'` columns as they made the original plot hard to interpret.
...
After analyzing each column's contents, the `'name'`, `'description'`, `'user_id'`, `'recipe_id'`, `'date'`, `'rating'`, `'review'`, and `'average_rating'` columns had NaN values. Using this information, I decided which columns needed to be cleaned based on the kind of missingness present. 

The `'description'` column had NaN values because it was optional for recipe authors to include a description of their recipe. The `'user_id'`, `'recipe_id'`, and `'date'` columns also had NaN values because a recipe could have had 0 reviews or ratings, meaning all of the columns from the previous `rating` dataset would appear as NaN for these rows. The `'rating'` column had NaN values because I replaced all ratings of 0 with np.nan. I also know that `'review'` can be NaN because a person can rate a recipe and not provide a review. Finally, the `'average_rating'` column can have NaN values because a recipe can have no ratings.

Out of these columns, I wanted to investigate the missingness of the `'name'`, `'description'`, `'rating'`, and `'review'` columns.

For the `'name'` column, I found that after copy-and-pasting the recipe's ID (stored within the `'id'` column) into food.com, the recipe was actually a salad dressing recipe with the title "-------------". Because these characters were stripped when the data was scraped, the name of the recipe was listed as a NaN value. Although the `'name'` column is not important, I filled this NaN value with a simple name, 'salad dressing'. I concluded that the reason why there was a singular NaN value within the `'name'` column was due to the original name itself. This means that the missingness of the `'name'` column is NMAR (Not Missing At Random), because the name itself (multiple dashes) caused itself to appear as NaN within the final cleaned dataset. This is because the developers of this dataset chose to omit unecessary characters such as dashes, resulting in this title appearing as a NaN value.
...
Once again, we can test the dependency of the `'rating'` column on another column. This time, I chose to test the dependency of the `'rating'` column on the `'minutes'` column.
...
By looking at the distribution, I noticed that the observed test statistic was not one that would appear within the distribution. Using an alpha value of 0.05 and a p-value of approximately 0.0, 0.0 < 0.05, meaning that my final model was not fair in its performance across Group X and Group Y. In other words, the F1-score is significantly different between the two groups, meaning that the model performs significantly better for Group X than for Group Y. This is because our observed test statistic is positive, suggesting that Group X's F1-score was greater than Group Y's F1-score, which is not due to chance as demonstrated by the graph above.

## Final Notes
It is important to note that the data utilized within this project was not a representative sample of all recipes created per year. After further analysis of the raw dataset, I had noticed that the majority of recipes were created in 2008, with a significant decline in recipe count as time progressed. The number of recipes within the dataset that were created in 2008 were in the ten-thousands, while the number of recipes created in 2018 did not even reach one-thousand. A potential cause of this issue may be due to the web-scraper that was utilized to collect the data, which only chose the top recommended recipes of each page, which happened to be older recipes.
